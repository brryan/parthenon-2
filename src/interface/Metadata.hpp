...
class Metadata {
 public:
  ...
  constexpr static Flag materials   = Flag(13); ///< 13: is specified per-material
  constexpr static Flag isotopes    = Flag(14); ///< 14: includes isotopes
  constexpr static Flag independent = Flag(15); ///< 15: is an independent, evolved variable
  constexpr static Flag derived     = Flag(16); ///< 16: is a derived quantity (ignored)
  constexpr static Flag oneCopy     = Flag(17); ///< 17: only one copy even if multiple stages
  constexpr static Flag fillGhost   = Flag(18); ///< 18: Do boundary communication
  constexpr static Flag sharedComms = Flag(19); ///< 19: Communication arrays are a copy: hint to destructor

  // `max` must always be the final flag - place all new flags prior to this flag
  static constexpr Flag max = Flag(20);
